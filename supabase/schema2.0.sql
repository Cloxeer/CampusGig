...
-- Alerts read public.notifications; RPCs write them here.
-- ============================================================
CREATE OR REPLACE FUNCTION private.notify_gig(
  p_user_id uuid, p_type text, p_title text, p_body text,
  p_gig_id uuid, p_request_id uuid, p_requester_id uuid, p_poster_id uuid,
  p_role text, p_other_name text
) RETURNS void
...
BEGIN
  ...
  VALUES (
    ...
    jsonb_strip_nulls(jsonb_build_object(
      'gig_id', p_gig_id, 'request_id', p_request_id, 'requester_id', p_requester_id,
      'poster_id', p_poster_id, 'role', p_role, 'other_name', p_other_name
    ))
  );
END;
$$;
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public', 'pg_temp' AS $$
DECLARE uid UUID := auth.uid(); g RECORD; rid UUID; pname TEXT; rname TEXT;
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.gig_requests WHERE gig_id = p_gig_id AND requester_id = uid) THEN
  ...
  PERFORM private.notify_gig(g.poster_id, 'gig_requested', rname || ' wants to take your gig', g.title, p_gig_id, rid, uid, g.poster_id, 'poster', rname);
  PERFORM private.notify_gig(uid, 'gig_request_sent', 'You requested a gig', g.title || ' · Waiting for ' || pname || ' to accept', p_gig_id, rid, uid, g.poster_id, 'requester', pname);
  RETURN rid;
END;
$$;
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public', 'pg_temp' AS $$
DECLARE uid UUID := auth.uid(); r RECORD; g RECORD; pname TEXT; pending_left INT;
BEGIN
  IF uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  SELECT * INTO r FROM public.gig_requests WHERE id = p_request_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Request not found'; END IF;
  SELECT * INTO g FROM public.gigs WHERE id = r.gig_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Gig not found'; END IF;
  IF g.poster_id <> uid THEN RAISE EXCEPTION 'Only the poster can reject'; END IF;
  IF r.status <> 'pending' THEN RAISE EXCEPTION 'Request is not pending'; END IF;
  ...
  END IF;
  pname := public._cg_display_name(uid);
  PERFORM private.notify_gig(r.requester_id, 'gig_rejected', 'Your gig request was declined', COALESCE(g.title, 'The poster chose someone else'), g.id, p_request_id, r.requester_id, g.poster_id, 'requester', pname);
END;
$$;

CREATE OR REPLACE FUNCTION private.complete_gig(p_gig_id UUID) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public', 'pg_temp' AS $$
DECLARE uid UUID := auth.uid(); g RECORD; tname TEXT; pname TEXT;
BEGIN
  IF uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  SELECT * INTO g FROM public.gigs WHERE id = p_gig_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Gig not found'; END IF;
  IF g.poster_id <> uid THEN RAISE EXCEPTION 'Only the poster can mark a gig as done'; END IF;
  ...
  IF g.taker_id IS NOT NULL THEN
    PERFORM private.notify_gig(g.taker_id, 'gig_completed', 'Gig completed! +10 Rep', g.title || ' · ' || pname || ' marked it done — you earned +10 Rep', p_gig_id, NULL, g.taker_id, g.poster_id, 'requester', pname);
  END IF;
  PERFORM private.notify_gig(uid, 'gig_completed', 'Gig marked as done! +8 Rep', CASE WHEN g.taker_id IS NOT NULL THEN g.title || ' · You earned +8 · ' || tname || ' earned +10' ELSE g.title || ' · +8 Rep' END, p_gig_id, NULL, g.taker_id, g.poster_id, 'poster', tname);
END;
$$;
